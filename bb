...
<body class="pb-32"> 
    ...
    <div class="max-w-md mx-auto min-h-screen bg-white shadow-xl relative overflow-hidden sm:border-x sm:border-gray-100">
                ...
                <div id="banner-slider" class="flex overflow-x-auto snap-x snap-mandatory gap-3 no-scrollbar rounded-2xl" style="scroll-behavior: smooth;">
                    ...
                    <div class="snap-center shrink-0 w-full h-40 relative rounded-2xl overflow-hidden shadow-md">
                        ...
                        <div class="absolute bottom-3 left-4 text-white">
                            <span class="bg-red-500 text-[10px] font-bold px-2 py-0.5 rounded-full mb-1 inline-block">HOT DEAL</span>
                            <p class="font-bold text-lg">ซื้อคู่ ถูกกว่า!</p>
                            <p class="text-xs opacity-90">จับคู่มะพร้าวปั่น + วุ้นกะทิ</p>
                        </div>
                    </div>
        ...
        <div id="sticky-footer" class="fixed bottom-0 w-full max-w-md bg-white/95 backdrop-blur shadow-[0_-4px_10px_rgba(0,0,0,0.05)] border-t p-4 z-40 hidden">
            ...
            <button id="btn-go-address" onclick="showPage('address')" class="hidden w-full bg-green-600 text-white rounded-xl p-3.5 font-bold shadow-lg hover:bg-green-700 transition">ระบุที่อยู่จัดส่ง</button>
            <button id="btn-confirm-address" onclick="validateAndGoToPayment()" class="hidden w-full bg-green-600 text-white rounded-xl p-3.5 font-bold shadow-lg hover:bg-green-700 transition disabled:bg-gray-400">ยืนยันที่อยู่ & ไปชำระเงิน</button>
    ...
    <script>
        ...
        // ⚙️ ตั้งค่าเวลาเปิด-ปิดร้าน (24H Format)
        const OPEN_HOUR = 9;   // เปิด 09:00
        const CLOSE_HOUR = 24; // ปิด 20:00

        // ข้อมูลสินค้า (เพิ่ม stock: true/false)
        const menuDB = [
            // Recommend
            { id: 1, category: "recommend", name: "มะพร้าวปั่น (ไม่ใส่นม)", desc: "น้ำมะพร้าวสดปั่นแท้ 100% ไม่ผสมน้ำเชื่อม", price: 65, originalPrice: 75, img: "https://images.unsplash.com/photo-1546171753-97d7676e4602?w=400", badge: "ขายดี #1", stock: true, options: [{ type: "radio", name: "ระดับความหวาน", choices: [{ label: "หวานธรรมชาติ", price: 0 }, { label: "หวานน้อย", price: 0 }, { label: "หวานปกติ", price: 0 }] }] },
            { id: 2, category: "recommend", name: "มะพร้าวปั่นนมสด", desc: "สูตรนมสด หอมมัน นัวสุดๆ", price: 119, originalPrice: 158, img: "https://images.unsplash.com/photo-1595981267035-7b04ca84a82d?w=400", stock: true, options: [{ type: "checkbox", name: "ท็อปปิ้ง (เลือกได้ 2)", max: 2, choices: [{ label: "ลอดช่องวัดเจษ", price: 15 }, { label: "ทับทิมกรอบ", price: 15 }, { label: "เนื้อมะพร้าวอ่อน", price: 10 }] }] },
            { id: 3, category: "recommend", name: "น้ำมะพร้าวสด (1 ลิตร)", desc: "น้ำมะพร้าวสดแท้ 100% แช่เย็นเจี๊ยบ", price: 293, originalPrice: 390, img: "https://images.unsplash.com/photo-1621506289937-a8e4df240d0b?w=400", stock: true },
            // Promotion
            { id: 4, category: "promotion", name: "Set คู่หูดูโอ้ (2 แก้ว)", desc: "มะพร้าวปั่นนมสด 2 แก้ว ราคาพิเศษ", price: 220, originalPrice: 238, badge: "ลดราคา 🔥", stock: true, img: "https://images.unsplash.com/photo-1628519592415-3174241e3d3f?w=400", options: [{ type: "radio", name: "ความหวาน (แก้ว 1)", choices: [{ label: "ปกติ", price: 0 }, { label: "น้อย", price: 0 }] }, { type: "radio", name: "ความหวาน (แก้ว 2)", choices: [{ label: "ปกติ", price: 0 }, { label: "น้อย", price: 0 }] }] },
            { id: 5, category: "promotion", name: "วุ้นมะพร้าวถ้วยยักษ์", desc: "วุ้นมะพร้าวทำเอง เนื้อแน่น หวานน้อย", price: 35, originalPrice: 50, badge: "Flash Sale", img: "https://images.unsplash.com/photo-1598211028080-1a65499c089c?w=400", stock: false } // ตัวอย่างสินค้าหมด
        ];
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // เริ่มระบบ Auto Slide
            startBannerAutoSlide();
            
            // 🧠 Remember Me: โหลดข้อมูลลูกค้าจาก LocalStorage
        ...
        function showPage(pageId) {
            ...
            // จัดการปุ่มย้อนกลับ (ซ่อนเมื่ออยู่หน้าเมนู)
            ...
            if (!isShopOpen()) {
                ...
                footer.innerHTML = `
                    <div class="w-full bg-gray-400 text-white rounded-xl p-3.5 text-center font-bold shadow-lg">
                        ⛔ ร้านปิด (เปิด ${String(OPEN_HOUR).padStart(2,'0')}:00 - ${String(CLOSE_HOUR).padStart(2,'0')}:00 น.)
                    </div>
                `;
                return; // หยุดการทำงาน ไม่โชว์ปุ่มอื่น
            }

            // ถ้าเปิดร้าน ให้โชว์ปุ่มปกติ (ต้อง Reset HTML กลับมาเป็นปุ่มเดิมก่อน)
            // หมายเหตุ: วิธีที่ดีคือแยก div แจ้งเตือนกับ div ปุ่ม แต่วิธีนี้ง่ายสุดสำหรับการแปะโค้ดทับ
            if(footer.innerHTML.includes('ร้านปิด')) location.reload(); // รีโหลดหน้านิดนึงเพื่อคืนค่าปุ่ม
            ...
            list.innerHTML = items.map(item => {
                ...
                <div class="bg-white p-4 border-b border-gray-100 flex gap-4 relative hover:bg-gray-50 transition active:bg-gray-100 cursor-pointer ${isSoldOut ? 'opacity-60 grayscale pointer-events-none' : ''}" onclick="${!isSoldOut ? `openModal(${item.id})` : ''}">
                    <div class="relative shrink-0 w-28 h-28">
                        ...
                        ${isSoldOut ? '<span class="absolute inset-0 flex items-center justify-center bg-black/40 text-white font-bold text-lg rounded-xl z-10">สินค้าหมด</span>' : ''}
                    </div>
                    
                    <div class="flex-1 flex flex-col justify-between py-1">
                        <div>
                            <h3 class="font-bold text-gray-800 text-base leading-tight mb-1">${item.name}</h3>
                            <p class="text-xs text-gray-500 line-clamp-2 leading-relaxed">${item.desc || 'อร่อย สดชื่น ต้องลอง!'}</p>
                        </div>
        ...
        function checkLimit(checkbox, max) {
            ...
            if (checkedCount > max) { checkbox.checked = false; Swal.fire({ toast: true, position: 'top', icon: 'info', title: `เลือกได้สูงสุด ${max} อย่าง`, showConfirmButton: false, timer: 1500 }); }
        }
        ...
        function updateCartUI() {
            ...
            if(cart.length === 0) { list.innerHTML = `<div class="text-center py-10"><i class="fa-solid fa-basket-shopping text-4xl text-gray-200 mb-2"></i><p class="text-gray-400">ยังไม่มีสินค้าในตะกร้า</p></div>`; return; }
            ...
            list.innerHTML = cart.map((item, idx) => {
                ...
                <div class="flex justify-between items-start border-b border-gray-100 pb-4 last:border-0 animation-fade-in">
                    <div class="flex-1 pr-2">
                        ...
                        <div class="mt-2"><span class="text-xs text-red-500 font-medium cursor-pointer hover:underline flex items-center gap-1 w-fit" onclick="cart.splice(${idx},1);updateCartUI()"><i class="fa-solid fa-trash-can"></i> ลบรายการ</span></div>
                    </div>
                    <span class="font-bold text-gray-800 whitespace-nowrap">฿${item.finalPrice}</span>
                </div>`;
            }).join('');
        }
        ...
        function validateAndGoToPayment() {
            ...
            if(deliveryFee === 0) {
                ...
                feeDetail.innerText = (distanceKm <= 1.5 && foodTotal >= 100) ? "(โปรใกล้บ้าน 1.5km)" : "(ครบ 200บ.)";
                feeDetail.className = "text-[10px] text-green-600";
            } else {
                feeDisplay.innerText = `${deliveryFee} บาท`; feeDisplay.className = "text-gray-800";
                if (distanceKm <= 1.5 && foodTotal < 100) {
                    feeDetail.innerText = `(สั่งเพิ่มอีก ${100 - foodTotal}บ. ส่งฟรี!)`; feeDetail.className = "text-xs text-red-500 font-bold animate-pulse";
                } else if (foodTotal < 200) {
                    feeDetail.innerText = `(สั่งเพิ่มอีก ${200 - foodTotal}บ. ส่งฟรี)`; feeDetail.className = "text-[10px] text-orange-500";
        ...
        async function confirmOrder() {
            ...
            // 🧠 Remember Me: บันทึกข้อมูลลูกค้าลงเครื่อง
            ...
            Swal.fire({ title: 'กำลังส่งออเดอร์...', text: 'กรุณารอสักครู่ ระบบกำลังแจ้งเตือนร้านค้า', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
            ...
            fetch(WEB_APP_URL, {
                ...
                Swal.fire({ icon: 'success', title: 'สั่งซื้อสำเร็จ!', text: 'ทางร้านได้รับข้อมูลเรียบร้อยแล้ว ขอบคุณครับ', confirmButtonColor: '#16a34a' }).then(() => location.reload());
            }).catch(err => {
                console.error(err); Swal.fire({ icon: 'error', text: 'เกิดข้อผิดพลาดในการส่งข้อมูล กรุณาลองใหม่' });
            });
        }
    ...
    let scrollAmount = 0;
    setInterval(() => {
        const cardWidth = slider.offsetWidth + 12; // ความกว้างการ์ด + gap
        scrollAmount += cardWidth;

        // ถ้าเลื่อนไปสุดแล้ว ให้กลับมาจุดเริ่มต้น
        if (scrollAmount >= slider.scrollWidth) {
            scrollAmount = 0;
            slider.scrollTo({ left: 0, behavior: 'auto' }); // วาร์ปกลับเร็วๆ
        } else {
            slider.scrollTo({ left: scrollAmount, behavior: 'smooth' }); // เลื่อนนุ่มๆ
        }
    }, 9000); // เลื่อนทุก 4 วินาที
}
    </script>
</body>
</html>
